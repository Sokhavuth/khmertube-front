...
export async function load({ locals, params, url, cookies }){
    ...
    const response = await fetch(`${locals.apiUrl}/api/admin/page/edit/${id}?amount=${settings.dashboard}&page=${navPage}`, {
        method: "GET",
        headers: {
            'X-User-Header': `${access_token}`
        }
    })
    const { count, page, items } = await response.json()
...
export const actions = {
    update: async ({ request, locals, cookies }) => {
	    ...
	    if(validate){
            ...
            const access_token = cookies.get('khmertube_access_token')
            const response = await fetch(`${locals.apiUrl}/api/admin/page/edit/${id}`,{
                method: "PUT",
